...
{
    [CreateAssetMenu(fileName = "ItemOrder", menuName = "Item/Order")]
    public class InteractableItemStackOrder :AbstractInteractableItemOrder
    {
        ...
        public override bool IsFeasibleOrder(List<InteractableItemType> order)
        {
            ...
            Debug.Log("All Types belong to order:" + eachTypeBelongsToOrder);
            if (eachTypeBelongsToOrder)
            {
                var firstItemIsValid = order.Count == 0 || allowDelayedStart || IsBottom(order[0]);
                Debug.Log("First Item Valid:" +firstItemIsValid);
                if (firstItemIsValid)
                {
